...
class RegisterEventAPIView(APIView):
    def post(self, request):
        ...
        activation_link = f"http://{current_site.domain}/accounts/api/activate/{uid}/{token}/"
        ...
        return Response({"message": "Emailga tadbirga aktivatsiya havolasi yuborildi."}, status=status.HTTP_201_CREATED)
...
class ActivationRegisterAPIView(APIView):
    def get(self, request, uidb64, token):
        try:
            ...
            return Response({"error": "Noto‘g‘ri aktivatsiya havolasi"}, status=status.HTTP_400_BAD_REQUEST)
        ...
        if default_token_generator.check_token(participant, token):
            ...
            return Response({"message": "Tadbirga ro'yxatdan o'tish muvaffaqiyatli faollashtirildi"}, status=status.HTTP_200_OK)
        else:
            return Response({"error": "Havola yaroqsiz yoki eskirgan"}, status=status.HTTP_400_BAD_REQUEST)
...
class LogoutEventAPIView(APIView):
    def post(self, request):
        ...
        if not event.participants.filter(pk=participant.pk).exists():
            return Response({'error': 'Siz bu tadbirda ro‘yxatdan o‘tmagansiz.'}, status=status.HTTP_400_BAD_REQUEST)
        ...
        return Response({"message": "Chiqish havolasi emailga yuborildi."}, status=status.HTTP_200_OK)

class ConfirmLogoutEventAPIView(APIView):
    def get(self, request, uidb64, token):
        slug = request.query_params.get('slug')
        if not slug:
            return Response({'error': 'Tadbir slug topilmadi'}, status=status.HTTP_400_BAD_REQUEST)

        try:
            uid = force_str(urlsafe_base64_decode(uidb64))
            participant = User.objects.get(pk=uid)
        except (User.DoesNotExist, ValueError, OverflowError, TypeError):
            return Response({"error": "Noto‘g‘ri havola"}, status=status.HTTP_400_BAD_REQUEST)

        if default_token_generator.check_token(participant, token):
            event = get_object_or_404(Event, slug=slug)
            try:
                participant_link = EventParticipant.objects.get(user=participant, event=event)
                participant_link.delete()
            except EventParticipant.DoesNotExist:
                return Response({"error": "Siz bu tadbirda ro‘yxatdan o‘tmagansiz"}, status=status.HTTP_400_BAD_REQUEST)

            return Response({"message": "Siz tadbirdan muvaffaqiyatli chiqdingiz."}, status=status.HTTP_200_OK)
        else:
            return Response({"error": "Havola yaroqsiz yoki eskirgan"}, status=status.HTTP_400_BAD_REQUEST)
